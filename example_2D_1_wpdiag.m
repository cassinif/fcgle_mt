...
for nsteps = mrange
  ...
  err_lbdf2m(counter) = sqrt(prod(h)*sum(abs(Ulbdf2m(:)-Uref(:)).^2));

  disp('LBDF2 V')
  [Ulbdf2v,cpu_lbdf2v(counter)] = lbdf2v(D,coeff_lin,g,U0(:),nsteps,tau);
  err_lbdf2v(counter) = sqrt(prod(h)*sum(abs(Ulbdf2v(:)-Uref(:)).^2));
end

figure;
loglog(err_lbdf2v,cpu_lbdf2v,'-or')
hold on
loglog(err_lbdf2m,cpu_lbdf2m,'-+b')
