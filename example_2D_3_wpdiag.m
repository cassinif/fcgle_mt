...
% Example 2D number 3: TDSRC with KROGSTAD using the proposed technique
...
for mu = 1:d
  ...
  gghat = 4/3*gg;
  idx = 1:2:n(mu);
  gghat(idx) = gghat(idx)-gg(1:length(idx))/(3*2^alpha(mu));
  D{mu} = toeplitz(gghat);
end
[X{1:d}] = ndgrid(x{:});
...
for nsteps = mrange
  ...
  disp('KROGSTAD M')
  [Ukm, cpu_km(counter)] = krogstadm(D,coeff_lin,G,U0,nsteps,tau);
  err_km(counter) = sqrt(prod(h)*sum(abs(Ukm(:)-Uref(:)).^2));

  xi = tau/10;
  disp('KROGSTAD V')
  [Ukv, cpu_kv(counter)] = krogstadv(D,coeff_lin,g,U0(:),nsteps,tau,m_k,xi);
  err_kv(counter) = sqrt(prod(h)*sum(abs(Ukv(:)-Uref(:)).^2));

end

figure;
loglog(err_kv,cpu_kv,'-or')
hold on
loglog(err_km,cpu_km,'-+b')
legend('KROGSTAD V','KROGSTAD M')
xlabel('Error')
